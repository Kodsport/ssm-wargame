...
func (knackKodenTeamL) LoadKnackKodenSolves(ctx context.Context, e boil.ContextExecutor, singular bool, maybeKnackKodenTeam interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`knack_koden_solves.knack_koden_team_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *KnackKodenTeam) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"knack_koden_teams\"", updateOnConflict, ret, update, conflict, insert)
