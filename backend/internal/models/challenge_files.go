...
func (o *ChallengeFile) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(challengeFileAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(challengeFilePrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert challenge_files, could not build conflict column list")
			}

			conflict = make([]string, len(challengeFilePrimaryKeyColumns))
			copy(conflict, challengeFilePrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"challenge_files\"", updateOnConflict, ret, update, conflict, insert, opts...)
