...
var (
	ctfAllColumns            = []string{"id", "name", "description", "slug", "private", "password", "created_at", "updated_at", "start_time", "end_time"}
	ctfColumnsWithoutDefault = []string{"id", "name", "description", "slug", "private", "start_time", "end_time"}
	ctfColumnsWithDefault    = []string{"password", "created_at", "updated_at"}
...
func (ctfL) LoadCTFChallenges(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCTF interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`ctf_challenges.ctf_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (ctfL) LoadCTFSolves(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCTF interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`ctf_solves`),
		qm.WhereIn(`ctf_solves.ctf_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (ctfL) LoadCTFUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCTF interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`ctf_users`),
		qm.WhereIn(`ctf_users.ctf_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *CTF) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(ctfPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert ctfs, could not build conflict column list")
			}

			conflict = make([]string, len(ctfPrimaryKeyColumns))
			copy(conflict, ctfPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"ctfs\"", updateOnConflict, ret, update, conflict, insert, opts...)
