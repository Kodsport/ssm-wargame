...
func (knackKodenSolfL) LoadKnackKodenTeam(ctx context.Context, e boil.ContextExecutor, singular bool, maybeKnackKodenSolf interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		args = append(args, object.KnackKodenTeamID)
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if a == obj.KnackKodenTeamID {
					continue Outer
				}
			}

			args = append(args, obj.KnackKodenTeamID)
	...
	query := NewQuery(
		...
		qm.WhereIn(`knack_koden_teams.id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *KnackKodenSolf) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"knack_koden_solves\"", updateOnConflict, ret, update, conflict, insert)
