...
func (o *ChallengeService) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(challengeServiceAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(challengeServicePrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert challenge_services, could not build conflict column list")
			}

			conflict = make([]string, len(challengeServicePrimaryKeyColumns))
			copy(conflict, challengeServicePrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"challenge_services\"", updateOnConflict, ret, update, conflict, insert, opts...)
