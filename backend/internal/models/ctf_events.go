...
var CTFEventRels = struct {
	Challenges             string
	ChalltoolsImportTokens string
}{
	Challenges:             "Challenges",
	ChalltoolsImportTokens: "ChalltoolsImportTokens",
}
...
type ctfEventR struct {
	...
	ChalltoolsImportTokens ChalltoolsImportTokenSlice `boil:"ChalltoolsImportTokens" json:"ChalltoolsImportTokens" toml:"ChalltoolsImportTokens" yaml:"ChalltoolsImportTokens"`
}
...
func (r *ctfEventR) GetChalltoolsImportTokens() ChalltoolsImportTokenSlice {
	if r == nil {
		return nil
	}
	return r.ChalltoolsImportTokens
}
...
// ChalltoolsImportTokens retrieves all the challtools_import_token's ChalltoolsImportTokens with an executor.
func (o *CTFEvent) ChalltoolsImportTokens(mods ...qm.QueryMod) challtoolsImportTokenQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"challtools_import_token\".\"ctf_event_id\"=?", o.ID),
	)

	return ChalltoolsImportTokens(queryMods...)
}
...
func (ctfEventL) LoadChallenges(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCTFEvent interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`challenges`),
		qm.WhereIn(`challenges.ctf_event_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// LoadChalltoolsImportTokens allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (ctfEventL) LoadChalltoolsImportTokens(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCTFEvent interface{}, mods queries.Applicator) error {
	var slice []*CTFEvent
	var object *CTFEvent
	...
	query := NewQuery(
		qm.From(`challtools_import_token`),
		qm.WhereIn(`challtools_import_token.ctf_event_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load challtools_import_token")
	}

	var resultSlice []*ChalltoolsImportToken
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice challtools_import_token")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on challtools_import_token")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for challtools_import_token")
	}
	...
	if singular {
		object.R.ChalltoolsImportTokens = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &challtoolsImportTokenR{}
			}
			foreign.R.CTFEvent = object
		}
		return nil
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.CTFEventID) {
				local.R.ChalltoolsImportTokens = append(local.R.ChalltoolsImportTokens, foreign)
				if foreign.R == nil {
					foreign.R = &challtoolsImportTokenR{}
				}
				foreign.R.CTFEvent = local
...
// AddChalltoolsImportTokens adds the given related objects to the existing relationships
// of the ctf_event, optionally inserting them as new records.
// Appends related to o.R.ChalltoolsImportTokens.
// Sets related.R.CTFEvent appropriately.
func (o *CTFEvent) AddChalltoolsImportTokens(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*ChalltoolsImportToken) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.CTFEventID, o.ID)
	...
	if o.R == nil {
		o.R = &ctfEventR{
			ChalltoolsImportTokens: related,
		}
	} else {
		o.R.ChalltoolsImportTokens = append(o.R.ChalltoolsImportTokens, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &challtoolsImportTokenR{
				CTFEvent: o,
			}
		} else {
			rel.R.CTFEvent = o
...
// SetChalltoolsImportTokens removes all previously related items of the
// ctf_event replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.CTFEvent's ChalltoolsImportTokens accordingly.
// Replaces o.R.ChalltoolsImportTokens with related.
// Sets related.R.CTFEvent's ChalltoolsImportTokens accordingly.
func (o *CTFEvent) SetChalltoolsImportTokens(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*ChalltoolsImportToken) error {
	query := "update \"challtools_import_token\" set \"ctf_event_id\" = null where \"ctf_event_id\" = $1"
	values := []interface{}{o.ID}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, query)
	...
	if o.R != nil {
		for _, rel := range o.R.ChalltoolsImportTokens {
			queries.SetScanner(&rel.CTFEventID, nil)
			if rel.R == nil {
				continue
			}
		...
		o.R.ChalltoolsImportTokens = nil
	}

	return o.AddChalltoolsImportTokens(ctx, exec, insert, related...)
}

// RemoveChalltoolsImportTokens relationships from objects passed in.
// Removes related items from R.ChalltoolsImportTokens (uses pointer comparison, removal does not keep order)
// Sets related.R.CTFEvent.
func (o *CTFEvent) RemoveChalltoolsImportTokens(ctx context.Context, exec boil.ContextExecutor, related ...*ChalltoolsImportToken) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		for i, ri := range o.R.ChalltoolsImportTokens {
			if rel != ri {
				continue
			}

			ln := len(o.R.ChalltoolsImportTokens)
			if ln > 1 && i < ln-1 {
				o.R.ChalltoolsImportTokens[i] = o.R.ChalltoolsImportTokens[ln-1]
			}
			o.R.ChalltoolsImportTokens = o.R.ChalltoolsImportTokens[:ln-1]
			break
		}
	}
...
func (o *CTFEvent) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"ctf_events\"", updateOnConflict, ret, update, conflict, insert)
