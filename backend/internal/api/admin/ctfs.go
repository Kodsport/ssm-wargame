...
func (s *service) CreateCTF(ctx context.Context, req *spec.CreateCTFPayload) (*spec.CTF, error) {
	...
	for _, challID := range req.ChallengeIds {
		_, err := s.db.ExecContext(ctx, `INSERT INTO ctf_challenges (ctf_id, challenge_id) VALUES ($1, $2)`, id, challID)
		if err != nil {
			return nil, err
		}
	}
...
func (s *service) UpdateCTF(ctx context.Context, req *spec.UpdateCTFPayload) (*spec.CTF, error) {
	...
	for _, challID := range req.ChallengeIds {
		_, err := s.db.ExecContext(ctx, `INSERT INTO ctf_challenges (ctf_id, challenge_id) VALUES ($1, $2)`, ctf.ID, challID)
		if err != nil {
			return nil, err
		}
	}
...
func (s *service) CreateChallengeGroup(ctx context.Context, req *spec.CreateChallengeGroupPayload) (*spec.ChallengeGroup, error) {
	...
	for _, challID := range req.ChallengeIds {
		_, err := s.db.ExecContext(ctx, `INSERT INTO challenge_group_challenges (challenge_group_id, challenge_id) VALUES ($1, $2)`, id, challID)
		if err != nil {
			return nil, err
		}
	}
...
func (s *service) UpdateChallengeGroup(ctx context.Context, req *spec.UpdateChallengeGroupPayload) (*spec.ChallengeGroup, error) {
	...
	for _, challID := range req.ChallengeIds {
		_, err := s.db.ExecContext(ctx, `INSERT INTO challenge_group_challenges (challenge_group_id, challenge_id) VALUES ($1, $2)`, group.ID, challID)
		if err != nil {
			return nil, err
		}
	}
...
func (s *service) ListChallengeGroups(ctx context.Context, req *spec.ListChallengeGroupsPayload) ([]*spec.ChallengeGroup, error) {
	...
	for _, group := range groups {
		rows, err := s.db.QueryContext(ctx, `SELECT challenge_id FROM challenge_group_challenges WHERE challenge_group_id = $1`, group.ID)
		if err != nil {
			return nil, err
		}
		var challengeIDs []string
