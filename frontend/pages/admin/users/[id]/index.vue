    ...
    <div v-if="user && !loading" class="container-fluid">
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <p v-if="user?.school_id"><strong>School:</strong> {{ user?.school_id }}</p>
                    ...
                    <div class="mt-2">
                        <div class="d-flex justify-content-center gap-3">
                            <div class="d-flex align-items-center">
                                <div class="bg-success" style="width: 12px; height: 12px; border-radius: 50%; margin-right: 4px;"></div>
                                <small>Correct: {{ submissionStats.successful }}</small>
                            </div>
                            <div class="d-flex align-items-center">
                                <div class="bg-danger" style="width: 12px; height: 12px; border-radius: 50%; margin-right: 4px;"></div>
        ...
        <div v-if="submissionStats.total > 0" class="mb-5">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="activity-chart-large d-flex align-items-end justify-content-center">
                        <div 
                        ...
                        >
                            ...
                            ></div>
                            <small class="hour-label-large">{{ hour.toString().padStart(2, '0') }}</small>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div v-for="challenge in challengeSubmissions" :key="challenge.challenge_id" class="mb-5 challenge-section">
            ...
            <div>
                <div class="table-responsive">
                    <table v-if="challenge.submissions.length" class="table">
                        ...
                        <tbody>
                            <tr v-for="submission in challenge.submissions" :key="submission.id">
                                <td class="text-break">{{ user?.email }}</td>
                                <td :class="{ 'bg-success': submission.successful, 'bg-danger': !submission.successful }" class="text-break">{{ submission.input }}</td>
                                <td>{{ new Date(submission.submitted_at * 1000).toString().split(' (')[0] }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
...
const discordAvatarUrl = computed(() => {
    ...
    if (user.value.discord_user?.avatar && !avatarError.value) {
        const format = user.value.discord_user.avatar.startsWith('a_') ? 'gif' : 'png'
...
const submissionStats = computed(() => {
    return userDetails.value?.submission_stats || {
        successful: 0,
        failed: 0,
        total: 0,
        success_rate: 0
...
const successCircumference = computed(() => {
    const percentage = submissionStats.value.total > 0 ? (submissionStats.value.successful / submissionStats.value.total) : 0
...
const failureCircumference = computed(() => {
    const percentage = submissionStats.value.total > 0 ? (submissionStats.value.failed / submissionStats.value.total) : 0
    return `${percentage * circumference} ${circumference}`
})

const failureOffset = computed(() => {
    const successPercentage = submissionStats.value.total > 0 ? (submissionStats.value.successful / submissionStats.value.total) : 0
...
onMounted(async () => {
    try {
        ...
        if (detailsPromise.status === 'fulfilled') {
            ...
            console.error(`Failed to load user details for ${userId}:`, detailsPromise.reason)
