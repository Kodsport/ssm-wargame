    ...
    <client-only>
      <section v-if="!auth.ctfUser.id">
        ...
        <form @submit.prevent="register">
          <input class="form-control" v-model="username" placeholder="Namn" required />
          <input v-if="ctfStore.ctf.team_based" class="form-control mt-2" v-model="teamCode" placeholder="Lagkod" />
        ...
        <form @submit.prevent="login">
          <input class="form-control" v-model="password" placeholder="correct-horse-battery-staple" required />
      ...
      <section v-if="auth.ctfUser.id">
        <p class="mb-2">
          Du är registrerad på denna CTF som {{ auth.ctfUser.username || ""
            ...
            i laget {{ auth.ctfUser.teamname }} </span>. Din lösenordsfras är
          <span id="passwordHover" class="hover" @click="copyPassword">{{
            auth.ctfUser.password || ""
          }}</span>. Kopiera eller kom ihåg den om du vill logga in på en annan enhet.
        </p>
        <button class="btn btn-primary me-2" @click="router.push(`/ctf/${slug}/challenges`)">
          Gå till utmaningar
        </button>
